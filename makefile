...
CSOURCE+=$(shell find csrc -name "*.c" -not -path "*/tools/capstone/repo/*" -not -path "csrc/ref/*")
CSOURCE+=$(shell find csrc -name "*.cc" -not -path "*/tools/capstone/repo/*" -not -path "csrc/ref/*")
...
VERILATOR_BASE_FLAGS = -DSIMULATION -cc $(VERILOG) --exe $(CSOURCE) --top-module $(TOP_NAME) -Mdir $(OBJ_DIR) -Ivsrc
